...
with app.app_context():
    ...
    record_columns = {"tags": "VARCHAR(400)", "thumbnail": "VARCHAR(400)", "source_type": "VARCHAR(80)", "source_url": "VARCHAR(400)", "is_featured": "BOOLEAN NOT NULL DEFAULT 0", "is_pinned": "BOOLEAN NOT NULL DEFAULT 0", "view_count": "INTEGER NOT NULL DEFAULT 0", "like_count": "INTEGER NOT NULL DEFAULT 0"}
...
def record_query():
    ...
    query = Archive.query
    if sort == "popular": query = query.order_by(Archive.like_count.desc(), Archive.view_count.desc(), Archive.created_at.desc())
...
def like_record(record_id):
    record = Archive.query.get_or_404(record_id); record.like_count += 1; db.session.commit()
